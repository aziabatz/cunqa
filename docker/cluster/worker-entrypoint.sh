...
## ESTE ES EL ENTRY DEL NODO WORKER/COMPUTE
...
mkdir -p /etc/slurm "$SLURMD_SPOOL_DIR"
...
echo "[worker] generating slurm.conf (cpus/node: ${CPUS_PER_NODE})"
...
echo "[worker] starting munged as user 'munge'"
su -s /bin/sh -c "/usr/sbin/munged" munge
for i in $(seq 1 30); do
  if [ -S /run/munge/munge.socket.2 ]; then
    echo "[worker] munge socket is ready"
    break
  fi
  echo "[worker] waiting for munge socket... ($i)"
  sleep 0.5
done

echo "[worker] starting slurmd"
# para escalar el cluster
exec slurmd -N "$(hostname)" -Dvv
