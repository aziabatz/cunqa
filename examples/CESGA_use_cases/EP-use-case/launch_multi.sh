...
if [ "$mode" == "cunqa" ]; then
    ...
    for ((n=2; n<=repetitions; n++)); do
        ...
        qraise -n $num_params -t 90:00:00 -c 1 --cloud --fakeqmio=/opt/cesga/qmio/hpc/calibrations/2025_04_02__12_00_02.json --family_name=cunqa-$n
        ...
        sbatch --job-name="$mode-$n" \
            ...
            -c $num_params \
            --mem-per-cpu=4G \
            --wrap="OMP_NUM_THREADS=$num_params python -u run-vqe-$mode.py --num_qubits \"$n\""
