...
void CustomJson::_write_MPI(json local_data)
{
    try {
        ...
        SPDLOG_LOGGER_DEBUG(loggie::logger, "JSON processed to write the data.");
        ...
        SPDLOG_LOGGER_DEBUG(loggie::logger, "Lengths gathered in order to calculate the displacements.");
        ...
        SPDLOG_LOGGER_DEBUG(loggie::logger, "Calculate displacements at root.");
        ...
        MPI_Gatherv(send_data, local_length, MPI_CHAR,
                    ...
                    0, MPI_COMM_WORLD);
        SPDLOG_LOGGER_DEBUG(loggie::logger, "Gather variable-length data at root.");
...
void CustomJson::_write_locks(json local_data)
{
    try {
        ...
        file_out << j.dump(4);
        file_out.close();
        SPDLOG_LOGGER_DEBUG(loggie::logger, "Succesfully written this process JSON into the file.");
