...
public:
    ...
    IPConfig(const std::string& hostname, std::unordered_map<std::string, std::string> IPs, std::string port)
        : hostname{hostname},
          IPs{IPs},
          port{port} { }
    
    IPConfig(json server_info)
    ...
    static IPConfig myIPConfig() {
        return IPConfig(get_hostname(), get_IP_addresses(), get_port());
    }
...
void from_json(const json& j, IPConfig& ipconfig) 
{
        
    j.at("hostname").get_to(ipconfig.hostname);
    for (auto& netbind : j.at("IPs").items()) {
        ipconfig.IPs[netbind.key()] = netbind.value();
    }
    j.at("port").get_to(ipconfig.port);
}; 
