...
{
    LOGGER_DEBUG("Setup QPUs arguments: argc={} argv= {} {}", argc, argv[1], argv[2]);
    ...
    try {
        ...
        if(auto search = SIM_NAMES.find(simulator); search != SIM_NAMES.end()) {
            if (search->second == SimType::Aer) {
                ...
                LOGGER_DEBUG("Turning ON the QPUs with the AER simulator.");
                qpu.turn_ON();
            } else if (search->second == SimType::Munich) {
                LOGGER_DEBUG("QPU_config: {}", qpu_config_json["noise"].dump(4));
                config::QPUConfig<SimType::Munich> qpu_config{qpu_config_json, info_path};
                LOGGER_DEBUG("QPU_config post qpu_config: {}", qpu_config.backend_config.noise_model.dump(4));
                QPU<SimType::Munich> qpu(qpu_config);
                LOGGER_DEBUG("Turning ON the QPUs with the Munich simulator.");
                qpu.turn_ON();
            }  
        } else
