...
int main(int argc, char* argv[]) 
{
    ...
    if (args.fakeqmio.has_value()) {
        if (args.fakeqmio.value() != "last_calibrations"){
            ...
            SPDLOG_LOGGER_DEBUG(loggie::logger, "FakeQmio last calibrations. Command: srun --task-epilog=$BINARIES_DIR/epilog.sh setup_qpus $INFO_PATH {} {}\n", args.simulator.c_str(), args.backend.value().c_str());
        } else {
            std::system(("source " + (std::string)std::getenv("INSTALL_PATH") + "/python/noise.sh " + args.fakeqmio.value()).c_str());
            ...
            SPDLOG_LOGGER_DEBUG(loggie::logger, "FakeQmio with calibrations path. Command: srun --task-epilog=$BINARIES_DIR/epilog.sh setup_qpus $INFO_PATH {} {}\n", args.simulator.c_str(), args.backend.value().c_str());
        }
    } else {
        if (args.backend.has_value()){
            sbatchFile << "srun --task-epilog=$BINARIES_DIR/epilog.sh setup_qpus $INFO_PATH " << args.simulator.c_str() << " " << args.backend.value().c_str() << "\n";

            SPDLOG_LOGGER_DEBUG(loggie::logger, "Qraise with backend. Command: srun --task-epilog=$BINARIES_DIR/epilog.sh setup_qpus $INFO_PATH {} {}\n", args.simulator.c_str(), args.backend.value().c_str());
        } else {
